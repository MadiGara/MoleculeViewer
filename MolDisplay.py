...
class Molecule(molecule.molecule):
    ...
    def parse(self, file):
        ...
        nums = [int(i) for i in lines[3].split() if i.isdigit()];
        atom_num = nums[0]; #atom_num is atom_max
        bond_num = nums[1];

        line_num = 4;
        # split each line into data elements by spaces
        while line_num < (atom_num + 4):
        ...
        while line_num < (atom_num + bond_num + 4):
            data = lines[line_num].split();
        ...
        if (self.atom_no < atom_num) or (self.bond_no < bond_num) or (atom_num == 0) or (bond_num == 0):
            print(f"Error! Atoms: {self.atom_no}, {atom_num}");
