...
void compute_coords (bond * bond) 
{
    ...
    bond->len = sqrt((pow((bond->x2 - bond->x1), 2.0)) + (pow((bond->y2 - bond->y1), 2.0)));
...
void mol_xform (molecule * molecule, xform_matrix matrix) 
{
    for (int i = 0; i < molecule->atom_no; i++)
    {
        // apply the transformation matrix to all atoms of molecule by performing vector matrix multiplication on the x, y, z coordinates
        double newX = (matrix[0][0] * molecule->atom_ptrs[i]->x) + (matrix[0][1] * molecule->atom_ptrs[i]->y) + (matrix[0][2] * molecule->atom_ptrs[i]->z); // x = row 0
        double newY = (matrix[1][0] * molecule->atom_ptrs[i]->x) + (matrix[1][1] * molecule->atom_ptrs[i]->y) + (matrix[1][2] * molecule->atom_ptrs[i]->z); // y = row 1
        double newZ = (matrix[2][0] * molecule->atom_ptrs[i]->x) + (matrix[2][1] * molecule->atom_ptrs[i]->y) + (matrix[2][2] * molecule->atom_ptrs[i]->z); // z = row 2
        // apply to atom
        molecule->atom_ptrs[i]->x = newX;
        molecule->atom_ptrs[i]->y = newY;
        molecule->atom_ptrs[i]->z = newZ;
    }
